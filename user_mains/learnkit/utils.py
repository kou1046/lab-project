...
def preprocess_keypoint(keypoint: models.KeyPoint, base_point: domain.KeyPointAttr) -> torch.Tensor:
    ...
    xs = np.array([point.x for point in keypoint.get_all_points()])
    ys = np.array([point.y for point in keypoint.get_all_points()])

    # 基準点の相対座標に変換
    relative_xs = xs - base_point.x
    relative_ys = ys - base_point.y

    max_x = relative_xs.max()
    max_y = relative_ys.max()

    # 正規化
    normalize_xs = np.divide(relative_xs, max_x, out=np.zeros_like(relative_xs), where=max_x != 0)
    normalize_ys = np.divide(relative_ys, max_y, out=np.zeros_like(relative_ys), where=max_y != 0)

    tensor = torch.from_numpy(np.array([normalize_xs, normalize_ys])).float().T
