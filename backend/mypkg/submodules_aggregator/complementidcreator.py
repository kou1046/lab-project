...
class ComplementIdCreator(tk.Tk):
    ...
    def __init__(
        ...
        if os.path.exists(os.path.join(out_dir, monitor_filename)):
            (
                ...
                os.path.join(out_dir, monitor_filename),
                os.path.join(out_dir, replace_filename),
                os.path.join(out_dir, stopresume_filename),
                os.path.join(out_dir, create_filename),
            )
            if self.create_ids:
    ...
    def _serialize_complement_ids(self) -> None:
        with open(os.path.join(self.out_dir, monitor_filename), "w") as f:
            json.dump(list(self.monitor_ids), f)
        with open(os.path.join(self.out_dir, replace_filename), "w") as f:
            json.dump(self.replace_ids, f)
        for key in self.stop_ids:
            self.stop_ids[key] = asdict(self.stop_ids[key])
        with open(os.path.join(self.out_dir, stopresume_filename), "w") as f:
            json.dump(self.stop_ids, f)
        for i, _ in enumerate(self.create_ids):
            self.create_ids[i] = asdict(self.create_ids[i])
        with open(os.path.join(self.out_dir, create_filename), "w") as f:
